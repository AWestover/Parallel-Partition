...
\author{Anonymous}
% \author{William Kuszmaul\thanks{Massachusetts Institute of Technology} \and Alek Westover\thanks{Massachusetts Institute of Technology}}
\date{}
...
algorithms due to its reliance on parallel partition.\footnote{In a
  ...
  not. CREW stands for \textit{concurrent-read exclusive-write}.}
...
performances. Finally, we conclude with open questions in
Appendix \ref{sec:open}. Figures and pseudocode are differed to Appendices
...
\textit{concurrent-read exclusive-write}. In practice, the important
...
\noindent\textit{The Parallel-Prefix Phase: }In this phase, the algorithm first
...
\noindent\textit{The Reordering Phase: }In this phase, the algorithm constructs
...
%% \alek{
  Recall that $G_i(j)$ denotes the index within $A$ of the $j$ th cache-line
  contained in $U_i$. By the definition of $G_i(j)$, $$(j - 1)g + 1 \le G_i(j)
  \le jg.$$ Note that $A[v_i]$ will occur in the $\lceil s\mu_i \rceil$-th
  cache-line of $U_i$ because $U_i$ is composed of $s$ cache lines. Hence
  $$(\lceil s\mu_i \rceil - 1) g b + 1 \le v_i \le \lceil s\mu_i \rceil g b,$$
  which means that
  $$s\mu_i g b - gb + 1 \le v_i \le s\mu_i g b + gb.$$ Since $sgb = n$, it
  follows that $|v_i - n \mu_i| \le gb$. Therefore, $$|v_i - n \mu| < gb +
  n\delta.$$
  This implies that the maximum of $|v_i - v_j|$ for any $i$ and $j$ is at
  most, $2bg + 2\delta n$. Thus,
\begin{align*}
...
machine; this is achieved using the \textit{interleave=all} option in
the Linux \textit{numactl} tool \cite{Kleen05}. Worker threads in our
...
array. Our code is compiled using g++ 7.3.0, with \textit{march=native}
...
\item \textit{A Serial Baseline:} This uses the serial in-place
...
\item \textit{The High-Space Parallel Implementation:} This uses the
  ...
  $2n$ eight-byte words.
\item \textit{The Medium-Space Parallel Implementation:} Starting with
  ...
  words.\footnote{In addition to the auxiliary array of size $n / 64$,
    ...
    practice.}
\item \textit{The Low-Space Parallel Implementation:}
...
We remark that the use of the Linux \textit{numactl} tool \cite{Kleen05}
...
\textit{numactl}, then the speedup of the Smoothed Striding Algorithm is
  ...
  \begin{algorithmic}% [1] for line numbers
    ...
    \Procedure{ParallelPartition}{$A$, $n$, $g$, $b$}
      ...
      \Else
        ...
        % \State // \textit{We implement the following parallel for-loop with recursive spawns}
        % \State // \textit{This facilitates computing $v_{min}$, $v_{max}$ without storing $v_y$'s}
...
\begin{figure*}
  ...
  \begin{algorithmic} % add [1] to get line numbers (if you want)
    ...
    \Procedure{ReadFromBlock}{$A$, $i$, $j$} \Comment Reads the value $v$ stored in $A[i], A[i+1], \ldots, A[j]$
      ...
      \Else
        \State \textit{Parallel-Spawn} $v_0 \gets $ ReadFromBlock($A$, $i$, $i+(j-i)/2$)
        \State \textit{Parallel-Spawn} $v_f \gets $ ReadFromBlock($A$, $i+(j-i)/2+1$, $j$)
        \State \textit{Parallel-Sync}
