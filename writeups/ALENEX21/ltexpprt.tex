...
We use Proposition \ref{prop:generalResult} as a tool to analyze the Recursive and the Hybrid Smoothed Striding Algorithms.
...
In Appendix \ref{sec:hybridandrecursivethmpfs} we prove the
following Theorem regarding the Hybrid Smoothed Striding Algorithm:
...
An interesting corollary of Theorem \ref{thm:fullPartition} concerns what
happens when $b$ is small (e.g., constant) and we choose $\delta$
to optimize span; the proof is in Appendix
\ref{sec:hybridandrecursivethmpfs}.
...
In Appendix \ref{sec:hybridandrecursivethmpfs} we also analyze
the Recursive Smoothed Striding Algorithm and prove the following
theorem:
...
algorithm occurs at $\delta = 1 / \sqrt{\log n}$; the proof of
Corollary \ref{cor:groupedPartitionAlg} is in Appendix
\ref{sec:hybridandrecursivethmpfs}.
...
\section{Recursive and Hybrid Smoothed Striding Algorithm
Analysis}
...
\begin{proof}[Proof of Theorem \ref{thm:fullPartition}]
  ...
  By Theorem \ref{thm:inplacesec}, the subproblem of partitioning of $A[v_{\text{min}} + 1], \ldots,
