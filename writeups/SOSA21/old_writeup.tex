...
$O(\log n \log \log n)$. In Appendix~\ref{secexp} we
experimentally evaluate the performance of implements of the
algorithms from this paper, along with the Strided Algorithm and
the Standard Algorithm. Finally, we conclude with open questions
in Section \ref{sec:open}. Figures and pseudocode are included in
...
\defn{Blocked-Prefix-Sum Partition Algorithm}, for parallel
...
Blocked-Prefix-Sum Partition Algorithm is useful as a subroutine
in the Smoothed Striding Algorithm. The Blocked-Prefix-Sum
Partition Algorithm has poor cache behavior, and so performs
somewhat poorly in practice. However, using it as a subroutine on
a small subproblem in the Smoothed Striding Algorithm allows
the Smoothed Striding Algorithm to achieve span $O(\log n
\log\log n)$ without sacrificing its cache behavior; we discuss
this in detail in Section~\ref{sec:hybridSmoothedStriding}. We now
describe the Blocked-Prefix-Sum Partition Algorithm. 
...
Blocked-Prefix-Sum Partition Algorithm. We show that doing so
results in an improved span (since the Blocked-Prefix-Sum
Partition Algorithm has span only $O(\log n \log \log n)$), while
still incurring only $(1 + o(1))n/b$ cache misses (since the
