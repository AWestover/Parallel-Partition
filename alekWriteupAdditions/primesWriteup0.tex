...
After partitioning the collection $U_y$, the Grouped Partition algorithm stores the index $v_y$ which is the index in $A$ of the first successor in $U_y$.
Recall that $s$ is small, and that the algorithm already created an array $X$ of size $s$, so storing the values $v_0,\ldots,v_{s-1}$ does not change the asymptotics for memory consumption of the algorithm.
After the Grouped Partition algorithm finishes performing in parallel a serial partition of each group, the algorithm computes in serial $v_{min} =\min v_y$ and $v_{max} = \max{v_y}$.
Note that we cannot just have a variable that all threads write to storing the current lowest $v_y$ and current highest $v_y$ and then overwrite this in each thread when it finishes its group because this would cause data races in the parallel algorithm.
