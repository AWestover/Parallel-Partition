...
make the classic Parallel Partition algorithm in-place.
The algorithm we describe here is useful as a subroutine in our main algorithm,
the Smoothed Striding Algorithm, which is discussed in Appendix
\ref{sec:smoothing}.
...
Smoothed Striding Algorithm allows it to achieve span $O(\log n \log\log n)$
without sacrificing it's cache behavior. 
...
positions $S[A[i]]$, the latter element will be a successor. In other
