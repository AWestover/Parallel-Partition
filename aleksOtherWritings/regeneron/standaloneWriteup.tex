...
\begin{abstract} The parallel-partition problem, which is the first step of
	...
	standard solution for the parallel-partition problem is out-of-place which
	prevents it from having spatial-locality.  Kuszmaul developed an in-place
	solution for the parallel-partition problem, but it performs multiple passes
	over the input array which prevents it from having temporal-locality.  The
	Blocked Strided Algorithm of Francis, Pannan, Frias, and Petit, is in-place
	and under certain conditions performs little more than a single pass over the
	array.  Because of this, for certain inputs the Blocked Strided Algorithm
	incurs very few cache misses and thus performs well.
	...
	algorithms, but which lacks theoretical guarantees on its span and cache
	behavior.
...
\note{need reference here?} By using linear-space the above parallel partition
algorithm incurs a large number of cache misses due to its lack of spatial
locality.  Kuszmaul modified the out-of-place parallel-prefix based algorithm
to make it in-place.  His new algorithm has span $O(\log n \log \log n)$ which
is close to the $O(\log n)$ span of the standard algorithm.  Because of its
spatial locality, Kuszmaul's in-place algorithm outperforms the standard
linear-space parallel partition algorithm in practice.  However experiments
also revealed that Kuszmaul's in-place algorithm was still bottlenecked by
cache misses.  Kuszmaul's algorithm performs multiple passes over the array, so
it incurs cache misses due to a lack of temporal locality.  The lack of
temporal locality means that memory bandwidth bound becomes a major factor in
determining whether further speedup is possible.



\section{A Cache Efficient In-Place Parallel Partition for \emph{Random} Input Arrays}\label{sec:strided}

In Section \ref{secexp}, we will see that, although Kuszmaul's in-place algorithm achieves speedups over the classic
parallel-prefix-based partition algorithm, it nonetheless continues
