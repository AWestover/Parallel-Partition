...
\begin{itemize}
	\item \defn{Serial Algorithm:} The serial partition algorithm algorithm serves as a baseline that we use to measure performance of parallel algorithms. 
	\item \defn{Standard Algorithm:} The standard linear-space parallel prefix sum based algorithm \footnote{The optimization of reducing the prefix-sum by a factor of $O(\log n)$ at the top level of recursion, rather than simply by a factor of two, can also be applied to the standard parallel-prefix algorithm when constructing a prefix-sum array of size $n$. Even without the space reduction, this reduces the (constant) overhead in the parallel prefix sum, while keeping the overall span of the parallel-prefix operation at $O(\log n)$. We perform this optimization in the high-space implementation.} 
	\item \defn{Kuszmaul's In-Place Algorithm:} Kuszmaul's in-place parallel prefix sum based algorithm. \footnote{The implementation of Kuszmaul's in-place algorithm still uses a small amount of auxiliary memory for the parallel-prefix sum, storing every $O(\log n)$-th element of the parallel-prefix array explicitly rather than using the implicit-storage approach originally developed by Kuszmaul. Nonetheless the space consumption is several orders of magnitude smaller than that of the Standard Linear-Space algorithm.}
	\item \defn{Blocked Strided Algorithm:} The Blocked Strided algorithm which only has guarantees for certain inputs such as random inputs. \footnote{We test this algorithm only on random inputs, which are among the few inputs on which it has theoretical guarantees.}
	\item \defn{Smoothed Striding Algorithm:} The algorithm developed in this paper which adds randomization to the Blocked Strided algorithm's internal structure to eliminate the need for specific input types.
\end{itemize}
We show that the algorithms which achieve space-reduction over the standard parallel-partition algorithm achieve empirical speedup.
We show that Kuszmaul's In-Place algorithm is bottlenecked by memory throughput and that the Blocked Strided and Smoothed-Striding algorithms are not bottlenecked by memory throughput.
...
parallel-for-loops, with one exception at the beginning of Kuszmaul's In-Place
Algorithm's implementation, when the number of predecessors in the input
...
2^{30}$. All space optimizations result in performance improvements, with
Kuszmaul's In-Place Algorithm's implementation performing almost four times as
well as the Standard Linear-Space Algorithm's implementation on eighteen
threads. %% Similar speedups are
...
on an input of size $2^{28}$, Kuszmaul's In-Place Algorithm's implementation and
the Standard Linear-Space Algorithm's implementation incur 305 million and 124 
...
read/write bandwidth $w$ bytes/second, the time needed for Kuszmaul's In-Place 
algorithm to perform its memory operations on an input of
$m$ bytes will be roughly $3.5 m / w + .5m / r$ seconds. We call this
the \defn{bandwidth constraint}. No matter how optimized an
implementation of the algorithm is, the bandwidth constraint
serves as a hard lower bound for the running time.
\footnote{Empirically, on an array of size $n = 2^{28}$, the total number of
cache misses is within $8\%$ of what this assumption would predict, suggesting
that the bandwidth constraint is within a small amount of the true
bandwidth-limited runtime.}

Figure \ref{tablebandwidth} compares the time taken by Kuszmaul's In-Place 
algorithm to its bandwidth constraint as the number of threads $t$
...
of auxiliary memory and achieve provable optimal cache-behavior (up to lower
order factors), while still using only exclusive read/write shared variables,
and maintaining a polylogarithmic span. The
